...
local function formatAvailableFormats(acquisitions, DownloadManager)
    ...
    for i, acquisition in ipairs(acquisitions) do
        if acquisition.count then
            -- PSE streaming
            table.insert(formats, _("Stream") .. " (" .. acquisition.count ..
                             " " .. _("pages") .. ")")
    ...
    if #formats == 0 then return _("None available") end
...
local function getPSEAcquisition(acquisitions)
    for _, acquisition in ipairs(acquisitions) do
        if acquisition.count then return acquisition end
    end
    return nil
end
        ...
        if not acquisition.count and acquisition.type ~= "borrow" then
            ...
            if filetype then
                table.insert(downloadable,
                             {acquisition = acquisition, filetype = filetype})
            end
        end
    end
    return downloadable
...
local function showFormatSelectionDialog(browser, item, downloadable,
                                         add_to_queue, parent_dialog)
    local DownloadManager = require("core.download_manager")
    local buttons = {}

    for _, dl in ipairs(downloadable) do
        local text = url.unescape(dl.acquisition.title or
                                      string.upper(dl.filetype))
        table.insert(buttons, {
            {
                ...
                callback = function()
                    ...
                    local local_path = DownloadManager.getLocalDownloadPath(
                                           browser, filename, dl.filetype,
                                           dl.acquisition.href)
                    local metadata = DownloadManager.buildDownloadMetadata(
                                         browser, item, dl.acquisition,
                                         dl.filetype)
    ...
    local title = add_to_queue and _("Select format to queue") or
                      _("Select format to download")

    browser.format_dialog = ButtonDialog:new{title = title, buttons = buttons}
...
function BookInfoDialog.build(browser, item)
    ...
    if item.author then base_filename = item.author .. " - " .. base_filename end
    if browser.root_catalog_raw_names then
        browser._custom_filename = nil
    else
        browser._custom_filename = browser._custom_filename or
                                       util.replaceAllInvalidChars(base_filename)
    end

    -- Get PSE and downloadable acquisitions
    local pse_acquisition = getPSEAcquisition(item.acquisitions)
    local downloadable = getDownloadableAcquisitions(item.acquisitions,
                                                     DownloadManager)
    ...
    if item.cover_bb or cover_link then
        ...
        cover_container.ges_events = {
            TapCover = {
                GestureRange:new{ges = "tap", range = cover_container.dimen}
            }
        }
        ...
        if cover_link then
            local function updateCoverWidget(content)
                ...
                local ok, hi_res_bb = pcall(function()
                    return RenderImage:renderImageData(content, #content, false,
                                                       target_width,
                                                       target_height)
                end)
            ...
            -- Start async load
            ImageLoader:loadImages({cover_link}, function(loaded_url, content)
                updateCoverWidget(content)
            end, browser.root_catalog_username, browser.root_catalog_password,
                                   browser.settings and
                                       browser.settings.cover_cache_enabled ~=
                                       false, browser.settings and
                                       browser.settings.cover_cache_max_mb,
                                   browser.settings and
                                       browser.settings.cover_cache_ttl_minutes)
        end
    end
    ...
    -- Author
    if item.author then
        table.insert(info_parts, {label = _("Author"), value = item.author})
    end
    ...
    for _, part in ipairs(info_parts) do
        table.insert(info_text_parts,
                     TextBoxWidget.PTF_BOLD_START .. part.label .. ":" ..
    ...
    if cover_container then
        header_content = HorizontalGroup:new{
            ...
            CenterContainer:new{
                dimen = Geom:new{
                    w = cover_width + Size.padding.default,
                    h = cover_height
                },
                cover_container
            },
    ...
    local header_height = cover_container and cover_height or
                              info_widget:getSize().h
    local button_height = Size.padding.large * 4 -- approximate for buttons
    local description_height =
        dialog_height - title_bar_height - header_height - button_height -
            Size.padding.large * 4
    ...
    local description_widget = ScrollableContainer:new{
        ...
        VerticalGroup:new{
            ...
            TextBoxWidget:new{
                text = TextBoxWidget.PTF_HEADER .. TextBoxWidget.PTF_BOLD_START ..
                    _("Description") .. TextBoxWidget.PTF_BOLD_END,
    ...
    if pse_acquisition then
        local stream_row = {
            {
                ...
                callback = function()
                    ...
                    OPDSPSE:streamPages(pse_acquisition.href,
                                        pse_acquisition.count, false,
                                        browser.root_catalog_username,
                                        browser.root_catalog_password)
                end
            }, {
        ...
        if pse_acquisition.last_read then
            ...
            table.insert(buttons_table, {
                text = Constants.ICONS.STREAM_RESUME .. " " .. _("Resume") ..
    ...
    if #downloadable > 0 then
        ...
        if #downloadable == 1 then
            ...
            table.insert(action_row, {
                text = Constants.ICONS.DOWNLOAD .. " " .. _("Download…"),
                callback = function()
                    showFormatSelectionDialog(browser, item, downloadable,
                                              false, browser.book_info_dialog)
                end
            })
        end
        ...
        if #downloadable == 1 then
            ...
            table.insert(action_row, {
                text = "+" .. " " .. _("Queue…"),
                callback = function()
                    showFormatSelectionDialog(browser, item, downloadable, true,
    ...
    if cover_link then
        table.insert(options_row,
                     {text = _("Full Cover"), callback = showFullCover})
    end
    ...
    table.insert(options_row, {
        text = _("Close"),
        callback = function() UIManager:close(browser.book_info_dialog) end
    })

    if #options_row > 0 then table.insert(buttons_table, options_row) end
    ...
    local dialog_frame = FrameContainer:new{
        ...
        padding = 0,
        margin = 0,
        VerticalGroup:new{align = "center", title_bar, content_frame}
    }
    ...
    browser.book_info_dialog.movable = MovableContainer:new{dialog_frame}
...
function BookInfoDialog.showDownloadOptionsDialog(browser, item)
    ...
    if item.author then filename_orig = item.author .. " - " .. filename_orig end
                            ...
                            {
                                {
                                    ...
                                    callback = function()
                                        ...
                                        if new_filename and new_filename ~= "" then
                                            ...
                                            browser._custom_filename =
                                                util.replaceAllInvalidChars(
                                                    new_filename)
                                            logger.dbg(
                                                "Custom filename set to:",
                                                browser._custom_filename)
                                        end
                                        UIManager:close(dialog)
                                    end
    ...
    browser.options_dialog = ButtonDialog:new{
        title = T(_("Download Options\n\nFolder: %1\n\nFilename: %2"),
                  BD.dirpath(current_dir), current_filename),
        buttons = buttons
    }
