...
function NavigationHandler.genItemTableFromCatalog(catalog, item_url,
                                                   browser_context,
                                                   debug_callback)
    local item_table = {}
    ...
    if not catalog then return item_table, facet_groups, search_url end
                ...
                if not browser_context.sync then
                    ...
                    if link.type:find(browser_context.search_type) then
                        if link.href then
                            ...
                            search_url = build_href(
                                             FeedFetcher.getSearchTemplate(
                    ...
                    if link.type:find(browser_context.search_template_type) and
                        link.rel and link.rel:find("search") then
                        if link.href and not has_opensearch then
                            search_url =
                                build_href(link.href:gsub("{searchTerms}", "%%s"))
                        end
                    end
                    ...
                    if link.rel == browser_context.facet_rel then
                        local group_name =
                            link["opds:facetGroup"] or _("Filters")
    ...
    for __, entry in ipairs(feed.entry or {}) do
        ...
        if entry.link then
            for ___, link in ipairs(entry.link) do
                ...
                if UrlUtils.isCatalogNavigationLink(link,
                                                    browser_context.catalog_type) then
                    item.url = link_href
                end
                ...
                if link.rel or link.title then
                    ...
                    elseif UrlUtils.isAcquisitionLink(link,
                                                      browser_context.acquisition_rel) then
                    ...
                    elseif link.rel == browser_context.stream_rel then
                        local count, last_read =
                            CatalogUtils.extractPSEStreamInfo(link)
                        if count then
                            table.insert(item.acquisitions, {
                                type = link.type,
                                href = link_href,
                    ...
                    elseif link.rel ~= "alternate" and
                        DocumentRegistry:hasProvider(nil, link.type) then
                        table.insert(item.acquisitions, {
                            type = link.type,
                            href = link_href,
                            title = link.title
                    ...
                    if link.title == "pdf" or link.type == "application/pdf" and
                        link.rel ~= "subsection" then
                        local original_href = link.href
                        local parsed = socket_url.parse(original_href)
                        if not parsed then
                            parsed = {path = original_href}
                        end
        ...
        local title = CatalogUtils.parseEntryTitle(entry.title,
                                                   _(Constants.DEFAULT_TITLE))
        item.text = title
        local author = CatalogUtils.parseEntryAuthor(entry.author, _(
        ...
        if (item.thumbnail or item.image) and item.acquisitions and
            #item.acquisitions > 0 then
        ...
        item.opds_updated = entry.updated
        item.opds_published = entry.published
...
function NavigationHandler.updateCatalog(item_url, browser, paths_updated)
    ...
    local debug_callback = function(...)
        if browser._debugLog then browser:_debugLog(...) end
    end

    local menu_table = FeedFetcher.genItemTableFromURL(item_url,
                                                       context.username,
                                                       context.password,
                                                       debug_callback,
    ...
    if #menu_table > 0 or browser.facet_groups or browser.search_url then
        ...
        if browser.facet_groups or browser.search_url then
            ...
            browser.title_bar_left_icon = cover_count > 0 and
                                              Constants.ICONS.MENU or
        ...
        if browser.page_num <= 1 then browser:onNextPage(true) end
...
function NavigationHandler.getNavigationDepth(browser) return #browser.paths end
...
function NavigationHandler.getCurrentUrl(browser)
    if #browser.paths > 0 then return browser.paths[#browser.paths].url end
    return nil
end
...
function NavigationHandler.getCurrentTitle(browser)
    if #browser.paths > 0 then return browser.paths[#browser.paths].title end
    return nil
end
...
function NavigationHandler.getBreadcrumbs(browser)
    ...
    for _, path in ipairs(browser.paths) do
        table.insert(breadcrumbs, {title = path.title, url = path.url})
    end
...
function NavigationHandler.isAtRoot(browser) return #browser.paths == 0 end
